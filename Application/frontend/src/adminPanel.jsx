...
export default function AdminPanel() {
    ...
    function sendGamepadData(gp = gamepad, currentSelector = selector, currentSpeed = vel) {
        const data = gp
            ? {
                Envirioment: { maxSpeed: currentSpeed },
                Setting: {
    ...
    function sendMap(gp) {
        ...
        const buttonInputs = mapContainer.querySelectorAll('.button-mapping');

        const mapData = {};

        // Botttoni
        gp.buttons.forEach((btn, index) => {
            const keyName = buttonInputs[index]?.value || `button_${index}`; // Prende il valore dall'input
            mapData[keyName] = btn.pressed ? true : false;
        });

        // Assi
        gp.axes.forEach((axis, index) => {
            const keyName = buttonInputs[gp.buttons.length + index]?.value || `axis_${index}`;
            mapData[keyName] = Number(axis.toFixed(2));
        });
                            ...
                            <div>
                                ...
                                {buttons.map((btn, index) => (
                                    <p key={index} className='p-mapping'>
                                        ...
                                        <strong>Value:</strong>{" "}
                                        <input type="text" className="button-mapping" />
                                    </p>
                                ))}
                                <h5>Assi</h5>
