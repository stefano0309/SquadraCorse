/*
 * Rx.ino — Ricezione Squadra Corse – Macchina 1
 *
 ...
 *
 * Driver motore: BST 7960
 *   RPWM (pin 1) → ESP32 D25  — avanti
 *   LPWM (pin 2) → ESP32 D33  — retro
 *   R_EN (pin 3) → 5V fisso
 *   L_EN (pin 4) → 5V fisso
 *   VCC  (pin 7) → 5V
 *   GND  (pin 8) → GND
...
// ==================== Motore (BST 7960) ====================
#define RPWM_PIN        25   // → BST pin 1 (RPWM) — avanti
#define LPWM_PIN        33   // → BST pin 2 (LPWM) — retro
#define PWM_FREQ      1000
#define PWM_RES          8   // 8 bit → 0-255
// ============== CONFIGURAZIONE ==============
#define STEER_ANGLE_MAX     88
...
#define SPEED_PCT_RATE      1.5f   // quanto veloce cambia speed_pct_smooth
// ── Soglia minima PWM per vincere l'attrito statico ──
#define MOTOR_MIN_PWM       25
// ── Coast-to-stop ──
#define COAST_HOLD_MS      2000    // ms prima di togliere tensione ai canali PWM
// ── PID Servo ──
...
void probeRadio() {
  ...
  if (stillAlive) { activeRadio = prev; return; }
...
void processPacket(const uint8_t *buf, int rssi) {
  if (buf[0] != TOKEN_0 || buf[1] != TOKEN_1 ||
      buf[2] != TOKEN_2 || buf[3] != TOKEN_3) {
    Serial.println("TOKEN_ERR");
    return;
  }
  uint16_t rxCrc   = ((uint16_t)buf[7] << 8) | buf[8];
  uint16_t calcCrc = crc16_ccitt(buf, 7);
  if (rxCrc != calcCrc) { Serial.println("CRC_ERR"); return; }
...
void setup() {
  ...
  // Motore BST 7960: due canali PWM indipendenti
...
void loop() {
  ...
  // ── Aggiornamento motore + servo PID (50 Hz) ──
  if (millis() - lastVelocityUpdate >= UPDATE_INTERVAL_MS) {
    ...
    if (fabs(pctDiff) < SPEED_PCT_RATE) speed_pct_smooth = pctTarget;
    else speed_pct_smooth += copysignf(SPEED_PCT_RATE, pctDiff);
    ...
    if (rx_freno) {
      ...
      if (velocita_attuale > 0.5f) velocita_target = 0;
      else velocita_target = -(rx_pressione_pedale / 255.0f) * max_pwm * REVERSE_MULTIPLIER;
    } else {
      if (velocita_attuale < -0.5f) velocita_target = 0;
      else velocita_target = (rx_pressione_pedale / 255.0f) * max_pwm;
    }

    // ═══ RAMPA VELOCITA ═══
    if (rx_freno) {
      if (fabs(velocita_attuale) <= DECEL_BRAKE) velocita_attuale = 0;
      else velocita_attuale -= copysignf(DECEL_BRAKE, velocita_attuale);
    } else {
    ...
    if (velocita_attuale > 0.5f) {
      // Avanti: RPWM attivo, LPWM a 0
      ledcWrite(RPWM_PIN, pwmVal);
      ledcWrite(LPWM_PIN, 0);
      lastDirection = 1;
      zeroReachedMs = 0;
    }
    else if (velocita_attuale < -0.5f) {
      // Retro: LPWM attivo, RPWM a 0
      ledcWrite(RPWM_PIN, 0);
      ledcWrite(LPWM_PIN, pwmVal);
      lastDirection = -1;
      zeroReachedMs = 0;
    }
    else {
      // Velocità ~0: coast-hold mantiene l'ultimo canale attivo
